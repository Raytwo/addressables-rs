...
use binrw::{until_eof, BinRead, BinResult, BinWrite};
...
impl BinWrite for KeyDataValue {
    ...
    fn write_options<W: std::io::Write + Seek>(&self, writer: &mut W, endian: binrw::Endian, args: Self::Args<'_>) -> BinResult<()> {
        match self {
            KeyDataValue::Hash(hash) => (4u8, hash).write_options(writer, endian, args),
            KeyDataValue::String { string, .. } => (0u8, string.len() as u32, string.as_bytes()).write_options(writer, endian, args),
        }
    }
}
...
impl BinWrite for ExtraValue {
    ...
    fn write_options<W: std::io::Write + Seek>(&self, writer: &mut W, endian: binrw::Endian, args: Self::Args<'_>) -> BinResult<()> {
        (
            7u8,
            self.assembly_name.len() as u8,
            self.assembly_name.as_bytes(),
            self.class_name.len() as u8,
            self.class_name.as_bytes(),
            self.json_text.len() as i32,
            &self.json_text.as_bytes(),
        )
